...
<body>
    ...
    <div class="header">
        <div>
            ...
            <div class="meta">N° {{ $devi->numero ?? ('DEV-'.$devi->id) }} · Date {{ $devi->date_devis ?? $devi->created_at?->toDateString() }}</div>
        </div>
        @php
            $logoFile = null;
    ...
    <div class="two-col">
        ...
        <div class="box" style="text-align:right;">
            <h4>À</h4>
            <div>{{ $devi->client->nom ?? 'Client' }}</div>
            <div>{{ $devi->client->email ?? '' }}</div>
            <div>{{ $devi->client->adresse ?? '' }}</div>
            <div>{{ $devi->client->telephone ?? '' }}</div>
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-end">{{ number_format($l->prix_unitaire ?? 0, 2, ',', ' ') }}</td>
    ...
    <div class="totaux">
        <div class="line"><span>Sous-total</span><span>{{ number_format($devi->total_ht ?? 0, 2, ',', ' ') }} {{ $settings['devise'] ?? 'FCFA' }}</span></div>
        <div class="line"><span>TVA</span><span>{{ number_format($devi->total_tva ?? 0, 2, ',', ' ') }} {{ $settings['devise'] ?? 'FCFA' }}</span></div>
        <div class="line total"><span>Total</span><span>{{ number_format($devi->total_ttc ?? 0, 2, ',', ' ') }} {{ $settings['devise'] ?? 'FCFA' }}</span></div>
    </div>
</body>
</html>
